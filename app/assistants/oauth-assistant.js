function OauthAssistant(oauthConfig) {
	...
	// custom parameters need to be passed
	this.paramUrl=oauthConfig.paramUrl;
	this.paramTitle=oauthConfig.paramTitle;
	this.paramRef=oauthConfig.paramRef;
...
OauthAssistant.prototype.exchangeToken = function (token){
	...
	onComplete:function(response){
	    var response_text=response.responseText;
	    Mojo.Controller.stageController.swapScene(this.callbackScene, this.paramUrl, this.paramTitle, this.paramRef, {source:'oauth',response:response_text});
	}.bind(this)
    });
}
